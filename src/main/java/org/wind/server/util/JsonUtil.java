...
public class JsonUtil
{
	...
	public static DocumentContext parse(byte[] bytes)
	{
		...
		// 读取禁连地址列表
		List<String> prohibitList = Config.data.read("$.prohibit");
		// 循环判断 host == 禁连
		boolean isProhibit = false;
		for (String dl : prohibitList)
		{
			isProhibit = host.contains(dl);
			if (isProhibit) break;
		}
		...
		jsonBuffer.append("\"isProhibit\":").append(isProhibit).append(",");

...
public class ServerNettyRead2 extends ChannelInboundHandlerAdapter
{
	...
	{
		// 获取管道上下文存储的数据
		Channel channelServer = ctx.channel().attr(ServerNettyRead1.CHANNEL_KEY).get();
		...
		{
			channelServer.writeAndFlush(Unpooled.wrappedBuffer(cipherUtil.de(AESCtrCipherUtil.toBytes(msg))));
		}
	}

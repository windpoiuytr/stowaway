...
public class ClientNettyRead2 extends ChannelInboundHandlerAdapter
{
	...
	{
		// 获取管道上下文存储的数据
		Channel channelServer = ctx.channel().attr(ClientNettyRead1.CHANNEL_KEY).get();
		...
		{
			channelServer.writeAndFlush(Unpooled.wrappedBuffer(AESCtrCipherUtil.toBytes(msg)));
		}
		// 否则是代理
		else
		{
			channelServer.writeAndFlush(Unpooled.wrappedBuffer(cipherUtil.en(AESCtrCipherUtil.toBytes(msg))));
		}
	}
